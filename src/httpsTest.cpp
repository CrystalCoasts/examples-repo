...
void sendPostRequest(String jsonPayload, const char* server, const char* resource) {
  Serial.println("Sending POST request...");
  sendData("AT+CSTT=" + String(apn));
  sendData("AT+CIICR");
  mySerial2.println("AT+CIFSR");
  ...
  while (millis() - startTime < timeout) {
    while (mySerial2.available()) {
      ...
      if (buffer0.find("72.250.81.62") != std::string::npos || buffer0.find("ERROR") != std::string::npos) {
        break;
      }
    }
  ...
  Serial.println(buffer0.c_str());

  sendData("AT+CIPSHUT");

  mySerial2.println("AT+CIPSTART=\"TCP\",\"128bdb57-9d10-4eb7-b3db-3aa86f885e1c.mock.pstmn.io\",\"80\"");
  startTime = millis();
  buffer0 = "";
  while (millis() - startTime < timeout) {
    while (mySerial2.available()) {
      ...
      // Check for termination keywords
      if (buffer0.find("CONNECT OK") != std::string::npos || buffer0.find("ERROR") != std::string::npos) {
        break;
      }
    }
    // Break if we already found "OK" or "ERROR"
  ...
  String builder = "";
  builder += "GET /get HTTP/1.1\n";
  builder += "Host: httpbin.org\n";
  builder += "User-Agent: esp-idf/1.0 esp32\n";
  builder += "Connection: keep-alive\n";

  builder = "POST /post HTTP/1.1\n";
  builder += "Host: 128bdb57-9d10-4eb7-b3db-3aa86f885e1c.mock.pstmn.io\n";
  builder += "Accept: */*\n";
  builder += "Content-Type: application/json\n";
  builder += "Content-Length: 0\n";
  builder += "Connection: keep-alive";

  builder = "POST /post HTTP/1.1\nHost: 128bdb57-9d10-4eb7-b3db-3aa86f885e1c.mock.pstmn.io\nAccept: */*\nContent-Type: application/json\nContent-Length: 81\n{\"Id\": 78912,\"Customer\": \"Jason Sweet\",\"Quantity\": 1,\"Price\": 18.0}";


//   builder += ("GET /?scope=images&nr=1 HTTP/1.1\n");
//   builder += ("Host: www.bing.com\n");
//   //builder.AppendLine("Content-Length: " + data.Length);   // only for POST request

  buffer0 = "";
  startTime = millis();
  mySerial2.println("AT+CIPSEND=" + String(builder.length() - 5)); //String(builder.length())
  Serial.println("AT+CIPSEND=" + String(builder.length() - 5));
 ...
 while (millis() - startTime < timeout) {
    while (mySerial2.available()) {
      ...
      // Check for termination keywords
      if (buffer0.find(">") != std::string::npos || buffer0.find("ERROR") != std::string::npos) {
        break;
      }
    }
    // Break if we already found "OK" or "ERROR"
  ...
  buffer0 = "";
  startTime = millis();
  mySerial2.println(builder + "0x1a");
  Serial.println(builder + "0x1a");
  while (millis() - startTime < timeout) {
    while (mySerial2.available()) {
      char c = mySerial2.read();
      buffer0 += c;
      ...
      // Check for termination keywords
      if (buffer0.find("SEND OK") != std::string::npos || buffer0.find("ERROR") != std::string::npos) {
        break;
      }
    }
    // Break if we already found "OK" or "ERROR"
  ...
  while (millis() - startTime < timeout) {
    while (mySerial2.available()) {
      ...
      // Check for termination keywords
      if (buffer0.find("CLOSED") != std::string::npos || buffer0.find("ERROR") != std::string::npos) {
        break;
      }
    }
    // Break if we already found "OK" or "ERROR"
